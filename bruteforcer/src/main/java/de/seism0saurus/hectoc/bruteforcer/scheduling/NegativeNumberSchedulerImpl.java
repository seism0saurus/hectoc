...
@Component
public class NegativeNumberSchedulerImpl implements NegativeNumberScheduler {
    ...
    private static final Logger LOGGER = new JobRunrDashboardLogger(LoggerFactory.getLogger(NegativeNumberSchedulerImpl.class));

    private final JobScheduler jobScheduler;

    public NegativeNumberSchedulerImpl(@Autowired JobScheduler jobScheduler) {
     ...
     */
    @Override
    public void scheduleNegativeNumberPermutations(HectocChallenge challenge, Stack<Number> stack, JobContext context) {
        final JobDashboardProgressBar progressBar = context.progressBar(MAX_PERMUTATIONS);
        scheduleNegativeNumberPermutations(challenge, stack, progressBar);
    }
     ...
     */
    private void scheduleNegativeNumberPermutations(HectocChallenge challenge, Stack<Number> stack, JobDashboardProgressBar progressBar) {
        NegativeNumberPermutator.createPermutationsOfNegativeNumbers(stack).stream()
                .peek(s -> {
                    final JobId enqueuedJobId = jobScheduler
                            .<OperatorScheduler>enqueue(
                                    UUID.nameUUIDFromBytes(("o_" + challenge + s.toString()).getBytes()),
                                    os -> os.scheduleOperatorPermutations(challenge, s, JobContext.Null)
                            );
